...
def analyze_sheets(selected_month, all_months):
...
def analyze_all_months(all_months):
    """Analyze all months with improved date parsing"""
    all_data = []
    emp_map, emp_designation_map = get_employee_map() # Correctly get the designation map
...
if month:
    df_all_time = analyze_all_months(all_months)
        ...
        if not df_all_time.empty:
            ...
            if selected_project_mom:
                mom_table, month_labels = create_month_on_month_project_table(
                    df_all_time, selected_project_mom, month, all_months
                )
