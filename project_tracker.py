...
if month:
    ...
    if df_summary.empty:
        ...
        #     # --- WEEKLY BREAKDOWN for selected project ---
        #     st.markdown("### 🗓️ Weekly Breakdown for Selected Project")

        #     # Copy of your code
        #     df_project_month = df_summary[df_summary["Project"] == selected_proj_for_table].copy()

        #     # 🔧 FIX 1: Remove rows with missing or invalid Date
        #     df_project_month = df_project_month[df_project_month["Date"].notna()]
        #     df_project_month = df_project_month[df_project_month["Date"].str.strip() != ""]

        #     def assign_week(date_str):
        #         try:
        #             # Parse format like 6/1/2025
        ...
        #     df_project_month["Week"] = df_project_month["Date"].apply(assign_week)
        #     # Filter out blank or zero columns before week assignment

        #     # First, identify the date columns
        #     date_cols = df_proj["Date"].unique()

        #     # If df_proj still has individual rows, we filter rows instead of columns
        #     df_proj = df_proj[df_proj["Hours"].notna() & (df_proj["Hours"].astype(str).str.strip().ne("")) & (df_proj["Hours"].astype(str).str.strip().ne("0")) & (df_proj["Hours"].astype(str).str.strip().ne("-"))]
        ...
        #     # Now assign week
        #     df_proj["Week"] = df_proj["Date"].apply(assign_week)

        #     # Remove any "Unknown" week rows
        #     df_proj = df_proj[df_proj["Week"] != "Unknown"]

        #     # Identify date columns to keep (non-empty columns)
        #     non_empty_cols = [col for col in date_cols if df[col].notna().sum() > 0 and df[col].astype(str).str.strip().ne("0").sum() > 0]

        #     # Keep only non-empty columns for analysis
        #     df_melt = df.melt(id_vars=[project_col], value_vars=non_empty_cols, var_name="Date", value_name="Hours")

        #     # Remove blanks, zeros, or dashes
        #     df_melt = df_melt[df_melt["Hours"].notna() & (df_melt["Hours"].astype(str).str.strip().ne("")) & (df_melt["Hours"].astype(str).str.strip().ne("0")) & (df_melt["Hours"].astype(str).str.strip().ne("-"))]

        #     # Convert Hours to numeric safely
        #     df_melt["Hours"] = pd.to_numeric(df_melt["Hours"], errors="coerce").fillna(0)

        #     # Only consider non-zero
        #     df_melt = df_melt[df_melt["Hours"] > 0]

        #     # Now assign week using assign_week function
        #     df_melt["Week"] = df_melt["Date"].apply(assign_week)

        #     # Remove any rows where week still 'Unknown' (edge case)
        #     df_melt = df_melt[df_melt["Week"] != "Unknown"]


        #     # 🔧 FIX 2: Remove 'Unknown' weeks
        #     df_project_month = df_project_month[df_project_month["Week"] != "Unknown"]

        #     # Weekly grouping
        #     weekly_grouped = df_project_month.groupby("Week")["Hours"].sum().reset_index()

        #     # Sort week labels
        #     week_order = ["Week 1", "Week 2", "Week 3", "Week 4"]
        #     weekly_grouped["Week"] = pd.Categorical(weekly_grouped["Week"], categories=week_order, ordered=True)
        #     weekly_grouped = weekly_grouped.sort_values("Week")
        ...
        #     )
        #     fig_week.update_traces(texttemplate='%{text:.1f}', textposition='outside')
        #     fig_week.update_layout(plot_bgcolor="#131313", paper_bgcolor="#0E1117", font=dict(color="white"))
        #     st.plotly_chart(fig_week, use_container_width=True)

        #     # Show table
        #     st.dataframe(weekly_grouped, use_container_width=True)
